...
const SeatSelectionClassiqScreen = () => {
  ...
  console.log('SeatSelection - Paramètres reçus:', route.params);

  // Normalisation des données pour voyage classique uniquement
  const tripData = {
  ...
  // États pour la sélection de sièges classiques uniquement
  ...
  useEffect(() => {
    setCurrentPrice(tripData.prixClassique * selectedSeats.length);
  }, [selectedSeats, tripData.prixClassique]);
  ...
  useEffect(() => {
    generateSeats();
  }, [reservedSeats, tripData.placesClassiqueTotal]);

  // Organisation des sièges par rangées
  ...
  const refreshData = async () => {
    ...
    try {
      ...
      if (doc.exists) {
        const data = doc.data();
        setReservedSeats(data.siegesReserves || []);
      }
    } catch (error) {
  ...
  const renderSeats = () => {
    const rows = organizeSeatsIntoRows(classiqueSeats);
    
    return (
      <View style={styles.seatLayout}>
        {/* En-tête du bus */}
        <View style={styles.busHeader}>
          <Text style={styles.busHeaderText}>Bus Classique</Text>
          ...
          row && Array.isArray(row) && (
            <View key={`row-${rowIndex}`} style={styles.seatRow}>
              {row.map((seat, index) => {
                ...
                // Ajouter un espace au milieu pour l'allée (après le 2ème siège)
                const showAisle = index === 1;
                ...
                return (
                  <React.Fragment key={seat.id}>
                    ...
                    {showAisle && <View style={styles.aisle} />}
                  </React.Fragment>
                );
              })}
            </View>
  ...
  const handleContinue = async () => {
    if (selectedSeats.length === 0) {
      Alert.alert('Aucun siège sélectionné', 'Veuillez sélectionner au moins un siège.');
      return;
    }
    ...
    try {
      ...
      const reservation = {
        ...
        prixClassique: tripData.prixClassique,
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Sélection des places</Text>
      ...
      <View style={styles.tripInfo}>
        <Text style={styles.tripInfoTitle}>Voyage Classique</Text>
        <Text style={styles.tripInfoPrice}>{formatPrice(tripData.prixClassique)} par siège</Text>
        <Text style={styles.tripInfoAvailability}>
          {tripData.placesClassiqueDisponibles} places disponibles sur {tripData.placesClassiqueTotal}
        </Text>
      </View>
      ...
      <View style={styles.footer}>
        ...
        >
          <Text style={styles.continueButtonText}>
            {isLoading 
              ? 'Traitement en cours...' 
              : `Continuer - ${selectedSeats.length} place${selectedSeats.length > 1 ? 's' : ''} (${formatPrice(currentPrice)})`
            }
          </Text>
        </TouchableOpacity>
      </View>
...
export default SeatSelectionClassiqScreen;